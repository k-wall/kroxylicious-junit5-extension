        ...
        Utils.awaitCondition(STARTUP_TIMEOUT, TimeUnit.SECONDS)
                .until(() -> {
                    ...
                    try {
                        ...
                        KafkaListener kafkaListener = this.getKafkaListener(Listener.EXTERNAL, configHolder.brokerNum());
                        KafkaListener kafkaListener1 = this.getKafkaListener(Listener.CONTROLLER, configHolder.brokerNum());
                        KafkaListener kafkaListener2 = this.getKafkaListener(Listener.EXTERNAL, configHolder.brokerNum());
                        KafkaListener kafkaListener3 = this.getKafkaListener(Listener.ANON, configHolder.brokerNum());
                        LOGGER.log(System.Logger.Level.WARNING, "anon: {0}, client: {1}, controller: {2}, interBroker: {3}, ",
                                kafkaListener3.bind().toString(),
                                kafkaListener2.bind().toString(),
                                kafkaListener1.bind().toString(),
                                kafkaListener.bind().toString());
